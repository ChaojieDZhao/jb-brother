...
public class RSAUtils
{
		...
		throws Exception
	{
		...
		for (String s : datas)
		{
			builder.append(bcd2Str(cipher.doFinal(s.getBytes("UTF-8"))));
		}
		...
		throws Exception
	{
		...
		for (byte[] arr : arrays)
		{
			builder.append(bcd2Str(cipher.doFinal(arr)));
		}
		return builder.toString();
	}
		...
		throws Exception
	{
		...
		//明文
		String ming = "why am i so handsome !!!";
		//使用模和指数生成公钥和私钥
		RSAPublicKey pubKey = RSAUtils.getPublicKey(modulus, public_exponent);
		RSAPrivateKey priKey = RSAUtils.getPrivateKey(modulus, private_exponent);
		//加密后的密文
		String encrypt = RSAUtils.encryptByPublicKey(ming, pubKey);
		System.err.println("encrypt=" + encrypt);
		//解密后的明文
		String decrypt = RSAUtils.decryptByPrivateKey(encrypt, priKey);
