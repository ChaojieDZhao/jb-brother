...
public class EncryptTool
{
	...
	{
		if ((null == args) || (args.length <= 0) || (args[0].equals(OPT_HELP)))
		{
			printHelp();
			return;
		}

		String encryptAlgorithm = DEFAULT_STRING_ENCRYPT;
	...
	{
		return encryptString(input, DEFAULT_KEY, DEFAULT_STRING_ENCRYPT, ENCRYPT_TYPE);
	}
	...
	public String encrypt(String input, String algorithm)
		...
		BadPaddingException, UnsupportedEncodingException
	{
		return encryptString(input, DEFAULT_KEY, algorithm, ENCRYPT_TYPE);
	}

	public String parseEncrypt(String input)
		throws InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, IllegalBlockSizeException,
		BadPaddingException, UnsupportedEncodingException
	{
		return encryptString(input, DEFAULT_KEY, DEFAULT_STRING_ENCRYPT, DECRYPT_TYPE);
	}

	public String parseEncrypt(String input, String algorithm)
		throws InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, IllegalBlockSizeException,
		BadPaddingException, UnsupportedEncodingException
	{
		return encryptString(input, DEFAULT_KEY, algorithm, DECRYPT_TYPE);
	}
		...
		BadPaddingException, UnsupportedEncodingException
	{
		...
		if ((null != input) && (0 != input.length()) && (null != key) && (0 != key.length()))
		{
			...
			{
				algorithm = DEFAULT_STRING_ENCRYPT;
			}

			byte[] raw = key.getBytes(UTFENCODING);
			...
			if (cryptionMode == 1)
			{
				byte[] outByte = cipher.doFinal(input.getBytes(UTFENCODING));
			...
			String strResult = new String(outByte, UTFENCODING);
			return strResult.trim();
		}
