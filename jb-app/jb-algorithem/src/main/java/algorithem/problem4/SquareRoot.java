...
public class SquareRoot
{
	...
	private static float sqrtRecursiveInner(float n, float low, float high)
	{
		...
		if (diff > DELTA)
			return sqrtRecursiveInner(n, low, sqrt);
		if (-diff > DELTA)
			return sqrtRecursiveInner(n, sqrt, high);
		return sqrt;
	}
	 ...
	 * as
	 * the previous fileDownload, but implemented using a tail recursive approach. In space terms, the
