...
public class BufferReaderDemo
{
	...
	{
		File file = new File(Const.RESOURCE_PATH + "/prose-poem.md");//当Test.java使用UTF-8编码，而使用GBK编码进行读取的时候会出现乱码；
		try
		{
	...
	{
		File file = new File(Const.RESOURCE_PATH + "/prose-poem.md");
		try
		{
			...
			BufferedReader in = new BufferedReader(new InputStreamReader(new FileInputStream(file),
				"UTF-8"));//使用本地环境中的默认字符集，例如在中文环境中将使用 GBK编码
			String line = null;
			while ((line = in.readLine()) != null)
			{
				System.out.println(line);
